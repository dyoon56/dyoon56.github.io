(function(factory) {
    ...
    function TaskQueue(debounceWait) {
        ...
        function addTask(taskFunc) {
            return new Promise(function(resolve) {
                var runFunc = function() {
                    Promise.resolve(taskFunc()).then(resolve).then(function() {
                        q.shift();
                        if (q.length) {
                            q[0]()
                        }
        ...
        this.add = typeof debounceWait === 'number' ? debounce(addTask, debounceWait) : addTask;
    ...
    var DayTableMixin = FC.DayTableMixin = {
        ...
        renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
            ...
            return '' + '<th class="' + classNames.join(' ') + '"' + (this.rowCnt === 1 ? ' data-date="' + date.format('YYYY-MM-DD') + '"' : '') + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + (otherAttrs ? ' ' + otherAttrs : '') + '>' + view.buildGotoAnchorHtml({
                date: date,
        ...
        renderNumberCellHtml: function(date) {
            ...
            if (this.view.dayNumbersVisible) {
                //html += this.view.buildGotoAnchorHtml(date, {
                  //  'class': 'fc-day-number'
//                }, date.date())
				html += '<span class=fc-day-number>'+date.date()+'</span>'
            }
            html += '</td>';
            return html
        },
    ...
    DayGrid.mixin({
        ...
        fgSegHtml: function(seg, disableResizing) {
            ...
            var titleHtml;
            if (event.school == 'IN') {
                classes.unshift('fc-day-grid-event', 'fc-h-event', 'school_in')
            } else {
                classes.unshift('fc-day-grid-event', 'fc-h-event', 'school_out')
            }
            if (seg.isStart) {
            ...
            return '<a class="' + classes.join(' ') + '"' +
                ...
                '>' +
                    '<input type="hidden" name="eventseq" value="'+event.key+'" />' +
    ...
    function Toolbar(calendar, toolbarOptions) {
        ...
        function render() {
            ...
            if (sections) {
                ...
                el.append(renderSection('left')).append('<div class="school_in_icon"><img src="../images/sub/event/school_in_icon.gif" alt="" />교내</div>').append('<div class="school_out_icon"><img src="../images/sub/event/school_out_icon.gif" alt="" />교외</div>').append(renderSection('right')).append(renderSection('center')).append('<div class="fc-clear"/>')
            } else {
                removeElement()
            }
        }
    ...
    function Calendar_constructor(element, overrides) {
        ...
        function renderView(viewType, forcedScroll) {
            ...
            if (currentView) {
                ...
                if (!currentView.isDateSet || !(date >= currentView.intervalStart && date < currentView.intervalEnd)) {
                    if (elementVisible()) {
                        if (forcedScroll) {
    ...
    var ListViewGrid = Grid.extend({
        ...
        renderSegList: function(allSegs) {
			...
			var j;
			var eventindex = [];
            var tableEl = $('<div><div/></div>');
            var tbodyEl = tableEl.find('div');
                ...
                if (daySegs) {
                  //  tbodyEl.append(this.dayHeaderHtml(this.view.start.clone().add(dayIndex, 'days')));
                    ...
                    for (i = 0; i < daySegs.length; i++) {
						tempIndex = $(daySegs[i].el).find("input[name=eventseq]").val();
						if(typeof eventindex[tempIndex]=="undefined" ){
							tbodyEl.append(this.dayHeaderHtml(this.view.start.clone().add(dayIndex, 'days')));
							tbodyEl.append(daySegs[i].el);
							eventindex[tempIndex] = 1;
						}
                    }
        ...
        dayHeaderHtml: function(dayDate) {
            ...
            return '<div class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' + '<div class="' + view.widgetHeaderClass + '" >' + (mainFormat ? view.buildGotoAnchorHtml(dayDate, {
                'class': 'fc-list-heading-main'
            }, htmlEscape(dayDate.format(mainFormat))) : '') + '</div>' + '</div>'
        },
        fgSegHtml: function(seg) {
            ...
            timeHtml = htmlEscape(this.getEventTimeText(event));
            if (event.school == 'IN') {
                classes.unshift('school_in')
            } else {
                classes.unshift('school_out')
            }
            if (url) {
                classes.push('fc-has-url')
            }
            return '<div class="' + classes.join(' ') + '">' +
                '<a href="#" class="fc-list-link">' +
                    '<input type="hidden" name="eventseq" value="'+event.key+'" />' +
                    (this.displayEventTime ?
                        '<div class="fc-list-item-time fl-r ' + view.widgetContentClass + '">' +
                            (timeHtml || '') +
                            '<p class="schedule_place">' +
                            event.place + 
                            '</p>' +
                        '</div>' : '') +
                    '<div class="fc-list-item-title fl-l ' + view.widgetContentClass + '">' +
                        '<h2 class="fc-link">' +
